...
npm version $BUMP -w apps/plugin --no-git-tag-version

# Extract the new version
VERSION=$(node -e "console.log(require('./apps/plugin/package.json').version)")

echo "Staging files..."
git add apps/plugin/package.json apps/plugin/manifest.json apps/plugin/versions.json package-lock.json
